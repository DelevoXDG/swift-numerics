...
public struct UInt128: Sendable {
  ...
  @usableFromInline @_transparent
  package init(low: UInt64, high: UInt64) {
#if _endian(little)
  ...
  @usableFromInline package var high: UInt64
#else
  @usableFromInline package var high: UInt64
  @usableFromInline package var low: UInt64
#endif
...
extension UInt128: Comparable {
  ...
  public static func <(a: Self, b: Self) -> Bool {
    // This "should" use the unlabeled init, but that's stdlib internal.
    // _builtinBooleanLiteral is identical and public.
    Bool(_builtinBooleanLiteral: Builtin.cmp_ult_Int128(a._value, b._value))
  }
}
...
extension UInt128 {
  ...
  public func addingReportingOverflow(
    ...
    let (newValue, overflow) = Builtin.uadd_with_overflow_Int128(
  ...
  public func subtractingReportingOverflow(
    ...
    let (newValue, overflow) = Builtin.usub_with_overflow_Int128(
      self._value, other._value, Builtin.zeroInitializer()
    )
    return (
      partialValue: Self(newValue),
  ...
  public func multipliedReportingOverflow(
    ...
    let (newValue, overflow) = Builtin.umul_with_overflow_Int128(
      self._value, other._value, Builtin.zeroInitializer()
    )
    return (
      partialValue: Self(newValue),
